...
const TipDetail = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/tips/article?url=${encodeURIComponent(tip.link)}`)
      .then((res) => {
  ...
  }, [tip?.link, tip?.fullContent, tip?.contentSnippet]);
        ...
        <main style={styles.main}>
          <div style={styles.articleCard}>
            <div style={styles.articleHeader}>
              ...
              {formattedDate && <p style={styles.articleMeta}>Published on {formattedDate}</p>}
            </div>
            ...
            {articleLoading ? (
              <div>
                {[100, 92, 97, 85, 90, 78, 95, 88, 100, 75, 93, 80].map((w, i) => (
                  <div key={i} style={{ ...styles.skeletonLine, width: `${w}%`, marginBottom: "12px" }} />
                ))}
              </div>
            ) : fullArticleHtml ? (
              <>
                ...
                <div className="tip-article-body" dangerouslySetInnerHTML={{ __html: fullArticleHtml }} />
              </>
            ) : (
              <div style={styles.emptyContent}>
                <p>No content is available for this article.</p>
                {tip.link && (
                  <a href={tip.link} target="_blank" rel="noopener noreferrer" style={styles.externalLink}>
            ...
            <div style={styles.articleFooter}>
              ...
              <button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} style={styles.scrollTopBtn}>
                ↑ Back to top
              </button>
            </div>
          </div>
