...
export default function MyDonations() {
  ...
  return (
    <div className="page-sm">
      ...
      <p className="muted">
        Track all the support you have provided to animals in need.
      </p>
            ...
            <tbody>
              {moneyHistory.map((don) => (
                <tr key={don._id} style={{ borderBottom: '1px solid #fafafa' }}>
                  <td style={{ padding: '10px', color: '#065f46', fontWeight: 'bold' }}>
                    {don.amount} TK
                  </td>
                  <td>{don.purpose}</td>
                  <td className="muted">
          ...
          <div className="grid" style={{ marginTop: 10 }}>
            {itemHistory.map((item) => (
              <div
                key={item._id}
                className="card"
                style={{ borderLeft: '4px solid #4f46e5', marginBottom: 10 }}
              >
